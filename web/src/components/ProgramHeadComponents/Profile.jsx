...
export default function Profile() {
  ...
  const handleSaveChanges = async () => {
    ...
    try {
      ...
      if (selectedImage) {
        ...
        const storageRef = ref(storage, `programHeadsPhoto/${folderId}/${selectedImage.name}`);
  ...
  if (isLoading) {
    return <div className="flex flex-1 items-center justify-center"><p>Loading Profile...</p></div>;
  }

  if (!currentUser || !profileData) {
    return <div className="flex flex-1 items-center justify-center"><p>Could not load profile. Please log in.</p></div>;
  }

  const fullName = `${profileData.firstName || ""} ${profileData.lastName || ""}`.trim();
  const currentPhotoURL = selectedImage ? URL.createObjectURL(selectedImage) : profileData.profileImage;
    ...
    return (
      <div className="w-full p-4 lg:p-6">
        <div className="flex justify-between mb-4">
          <Button variant="outline" className="cursor-pointer text-sm gap-2" onClick={() => setIsEditing(false)}>
        ...
        <div className="flex flex-col lg:flex-row gap-6 items-start">
          ...
          <Card className="w-full max-w-sm mx-auto lg:mx-0">
            ...
            <CardContent className="p-4 flex flex-col items-center">
              <div className="relative w-36 h-36 mb-4 rounded-full group cursor-pointer" onClick={() => fileInputRef.current?.click()}>
                {currentPhotoURL ? (
                  <img src={currentPhotoURL} alt="Profile" className="w-full h-full rounded-full border-4 border-white shadow-md object-cover" />
                ) : (
            ...
            <Card>
              ...
              <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="flex flex-col gap-1">
                  ...
                  <Input value={formData.role || formData.positionTitle || formData.position || "Program Head"} disabled />
                </div>
              </CardContent>
            </Card>
          </div>
        ...
        <div className="flex justify-end gap-2 mt-4">
          <Button variant="ghost" onClick={() => setIsEditing(false)} className="cursor-pointer">Cancel</Button>
        ...
        <Card className="w-full max-w-sm mx-auto lg:mx-0">
          <CardContent className="p-6 flex flex-col items-center">
            {profileData.profileImage ? (
              <img src={profileData.profileImage} alt="Profile" className="w-28 h-28 rounded-full border-4 border-white shadow-md object-cover" />
            ) : (
          ...
          <Card>
            ...
            <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <p>{profileData.role || profileData.positionTitle || profileData.position || "Program Head"}</p>
              </div>
            </CardContent>
          </Card>
        </div>
