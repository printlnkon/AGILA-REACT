...
export default function AddYearLevelModal() {
  ...
  const handleAddYearLevel = async () => {
    ...
    try {
      // Check if year level already exists (case-insensitive check)
      const yearLevelsRef = collection(db, "year_levels");
      const q = query(
        yearLevelsRef,
        where("name_lowercase", "==", yearLevelName.trim().toLowerCase())
      );
      ...
      await addDoc(collection(db, "year_levels"), {
        name: yearLevelName.trim(),
        name_lowercase: yearLevelName.trim().toLowerCase(),
