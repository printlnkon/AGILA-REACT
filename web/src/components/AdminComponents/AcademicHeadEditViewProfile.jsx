...
export default function AcademicHeadEditViewProfile({
  academicHead,
  onSave,
  onCancel,
}) {
  ...
  const handleSaveChanges = async () => {
    ...
    const updatedAcademicHeadData = { ...formData };

    if (selectedImage) {
      const storageRef = ref(
        storage,
        `academicHeadsPhoto/${academicHead.employeeNumber}/${selectedImage.name}`
      );
      await uploadBytes(storageRef, selectedImage);
      const photoURL = await getDownloadURL(storageRef);
      updatedAcademicHeadData.photoURL = photoURL;
    }

    onSave(updatedAcademicHeadData);
  };

  if (!formData) {
    return (
      <div className="flex items-center justify-center h-full p-4">
        <p className="text-muted-foreground">Loading academic head data...</p>
      </div>
    );
  }
      ...
      <div className="flex flex-col lg:flex-row gap-6 items-start">
            ...
            <div className="relative w-36 h-36 mb-4 rounded-full group">
              <img
                ...
                onError={(e) => {
                  ...
                  e.target.src =
                    "https://placehold.co/144x144/b6e3f4/4a4a4a?text=AcademicHead";
                }}
              />
              <div
        ...
        <div className="w-full flex flex-col gap-4">
          <Card className="w-full">
            <CardHeader>
              <div className="font-semibold text-lg sm:text-xl">
                Edit Academic Head Information
              </div>
            </CardHeader>
            <CardContent>
