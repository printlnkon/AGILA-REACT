...
def upload_to_firebase_storage(image_data, user_name, unique_number, image_index):
    try:
        safe_name = user_name.replace(" ", "_").lower()
        safe_number = str(unique_number).strip()
        file_path = f"faces/{safe_name}/{safe_number}/face_{image_index}.jpg"
...
def register_face():
    try:
        ...
        for idx, base64_img in enumerate(images):
            ...
            url = upload_to_firebase_storage(img_bytes, name, unique_number, idx)
            if url:
